...
in
{
  ...
  testScript = ''
    ...
    client.fail(
        f"ssh {MOCKUSER}@server '${config.security.wrapperDir}/sudo ${config.environment.systemDir}/sw/bin/id' | grep -q 'root'"
    )
    ...
    client.succeed(
        f"ssh {MOCKADMIN}@server '${config.security.wrapperDir}/sudo ${config.environment.systemDir}/sw/bin/id' | grep -q 'root'"
    )
  '';
}
