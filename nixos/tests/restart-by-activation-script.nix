  ...
  testScript = # python
    ''
      ...
      with subtest("nothing happens when the activation script does nothing"):
          out = machine.succeed("${config.environment.systemDir}/bin/switch-to-configuration dry-activate 2>&1")
          assert 'restart' not in out
          assert 'reload' not in out
          out = machine.succeed("${config.environment.systemDir}/bin/switch-to-configuration test")
          assert 'restart' not in out
          assert 'reload' not in out
      ...
      with subtest("real activation"):
          out = machine.succeed("${config.environment.systemDir}/bin/switch-to-configuration test 2>&1")
